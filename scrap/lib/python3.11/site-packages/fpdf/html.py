...
class HTML2FPDF(HTMLParser):
    ...
    def handle_data(self, txt):
        if self.td is not None:   # drawing a table?
            ...
            bgcolor = hex2dec(
                self.td.get('bgcolor', self.tr.get('bgcolor', ''))
            )
            ...
            if (
                self.pdf.y + height > self.pdf.page_break_trigger
                and not self.th
            ):
            ...
            if DEBUG:
                print('cell', txt, '*')
            self.pdf.cell(
                0, self.h, txt, 0, 1, self.align[0].upper(), self.href
            )
        else:
            txt = txt.replace('\n', ' ')
            if self.href:
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'img':
            if 'src' in attrs:
                ...
                self.pdf.image(
                    self.image_map(attrs['src']), x, y, w, h, link=self.href
                )
                self.pdf.set_x(x + w)
                self.pdf.set_y(y + h)
        if tag == 'b' or tag == 'i' or tag == 'u':
