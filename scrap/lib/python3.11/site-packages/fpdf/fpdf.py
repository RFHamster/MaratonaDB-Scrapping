...
class FPDF(object):
    ...
    def _set_dash(self, dash_length=False, space_length=False):
        if dash_length and space_length:
            s = sprintf(
                '[%.3f %.3f] 0 d', dash_length * self.k, space_length * self.k
            )
        else:
            s = '[] 0 d'
        self._out(s)
        ...
        if fontkey not in self.fonts:
            ...
            if fontkey in self.core_fonts:
                if fontkey not in fpdf_charwidths:
                    ...
                    exec(
                        compile(
                            open(name + '.font').read(), name + '.font', 'exec'
                        )
                    )
                    if fontkey not in fpdf_charwidths:
                        self.error(
    ...
    @check_page
    def multi_cell(
        self, w, h, txt='', border=0, align='J', fill=0, split_only=False
    ):
        ...
        while i < nb:
            ...
            if l > wmax:
                ...
                if sep == -1:
                    ...
                    if not split_only:
                        self.cell(
                            w, h, substr(s, j, sep - j), b, 2, align, fill
                        )
                    else:
    ...
    def write(self, h, txt='', link=''):
        ...
        if i != j:
            self.cell(
                l / 1000.0 * self.font_size, h, substr(s, j), 0, 0, '', 0, link
            )
    ...
    def _putTTfontwidths(self, font, maxUni):
        if font['unifilename']:
            cw127fname = (
                os.path.splitext(font['unifilename'])[0] + '.cw127.pkl'
            )
        else:
            cw127fname = None
        ...
        if 'data' in info:
            ...
            if 'trns' in info and isinstance(info['trns'], list):
                ...
                for i in range(0, len(info['trns'])):
                    trns += (
                        str(info['trns'][i]) + ' ' + str(info['trns'][i]) + ' '
                    )
                self._out('/Mask [' + trns + ']')
            if 'smask' in info:
    ...
    def _putcatalog(self):
        ...
        if self.zoom_mode == 'fullpage':
            ...
            self._out(
                sprintf(
                    '/OpenAction [3 0 R /XYZ null null %s]',
                    self.zoom_mode / 100,
                )
            )
            ...
            while True:
                ...
                if markerHigh != 0xFF or markerLow < 0xC0:
                    ...
                    if (
                        ...
                        or (  # SOF0 - SOF3
                            markerLow >= 0xC5 and markerLow <= 0xC7
                        )
                        or (  # SOF4 - SOF7
