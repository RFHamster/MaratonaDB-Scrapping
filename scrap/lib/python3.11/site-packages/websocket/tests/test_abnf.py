...
class ABNFTest(unittest.TestCase):
    ...
    def test_validate(self):
        ...
        a_bad_close_frame = ABNF(
            0, 0, 0, 0, opcode=ABNF.OPCODE_CLOSE, data=b'\x01'
        )
        self.assertRaises(
            WebSocketProtocolException,
    ...
    def test_format(self):
        ...
        abnf_length_10 = ABNF(
            0, 0, 0, 0, opcode=ABNF.OPCODE_TEXT, data='abcdefghij'
        )
        self.assertEqual(
            b'\x01', abnf_length_10.format()[0].to_bytes(1, 'big')
        )
        self.assertEqual(
            b'\x8a', abnf_length_10.format()[1].to_bytes(1, 'big')
        )
        self.assertEqual(
            'fin=0 opcode=1 data=abcdefghij', abnf_length_10.__str__()
        )
        abnf_length_20 = ABNF(
            0, 0, 0, 0, opcode=ABNF.OPCODE_BINARY, data='abcdefghijabcdefghij'
        )
        self.assertEqual(
            b'\x02', abnf_length_20.format()[0].to_bytes(1, 'big')
        )
        self.assertEqual(
            b'\x94', abnf_length_20.format()[1].to_bytes(1, 'big')
        )
        abnf_no_mask = ABNF(
            0,
            0,
