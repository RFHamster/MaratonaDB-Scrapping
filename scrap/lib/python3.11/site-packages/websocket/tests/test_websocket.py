...
class WebSocketTest(unittest.TestCase):
    ...
    def test_ws_utils(self):
        ...
        self.assertEqual(
            _validate_header(required_header, key, None), (True, None)
        )
        ...
        self.assertEqual(
            _validate_header(header, key, ['sub2', 'sub3']), (False, None)
        )
        ...
        # This case will print out a logging error using the error() function, but that is expected
        self.assertEqual(
            _validate_header(header, key, ['Sub1', 'suB2']), (False, None)
        )
    ...
    @unittest.skipUnless(
        TEST_WITH_INTERNET, 'Internet-requiring tests are disabled'
    )
    def test_iter(self):
        count = 2
    ...
    def test_recv_with_prolonged_fragmentation(self):
        ...
        s.add_packet(
            b'\x00\x8eabcd\x05\x07\x02\x16A\x04\x11\r\x04\x0c\x07\x17MB'
        )
        # OPCODE=CONT, FIN=1, MSG="once more"
        s.add_packet(b'\x80\x89abcd\x0e\x0c\x00\x01A\x0f\x0c\x16\x04')
        data = sock.recv()
        self.assertEqual(
            data, 'Once more unto the breach, dear friends, once more'
        )
        with self.assertRaises(ws.WebSocketConnectionClosedException):
            sock.recv()
    ...
    def test_recv_with_fragmentation_and_control_frame(self):
        ...
        s.add_packet(
            b'\x80\x8fabcd\x0e\x04C\x05A\x05\x0c\x0b\x05B\x17\x0c\x08\x0c\x04'
        )
        data = sock.recv()
    ...
    @unittest.skipUnless(
        ...
        'Tests using local websocket server are disabled',
    )
    ...
    def test_after_close(self):
        ...
        s.close()
        self.assertRaises(
            ws.WebSocketConnectionClosedException, s.send, 'Hello'
        )
...
class HandshakeTest(unittest.TestCase):
    ...
    def test_http_ssl(self):
        ...
        self.assertRaises(
            ValueError, websock1.connect, 'wss://api.bitfinex.com/ws/2'
        )
