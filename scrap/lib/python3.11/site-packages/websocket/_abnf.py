...
try:
    ...
    def _mask(mask_value: array.array, data_value: array.array) -> bytes:
        ...
        return (int_data_value ^ int_mask_value).to_bytes(
...
class ABNF:
    ...
    @staticmethod
    def create_frame(
        data: Union[bytes, str], opcode: int, fin: int = 1
    ) -> 'ABNF':
        """
    ...
    def format(self) -> bytes:
        ...
        if length < ABNF.LENGTH_7:
            frame_header += chr(self.mask_value << 7 | length).encode(
                'latin-1'
            )
...
class continuous_frame:
    def __init__(
        self, fire_cont_frame: bool, skip_utf8_validation: bool
    ) -> None:
