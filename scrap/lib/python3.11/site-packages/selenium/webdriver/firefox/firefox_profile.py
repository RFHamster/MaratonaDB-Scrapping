    ...
    def __init__(self, profile_directory=None):
        ...
        if profile_directory:
            newprof = os.path.join(
                tempfile.mkdtemp(), 'webdriver-py-profilecopy'
            )
            shutil.copytree(
                profile_directory,
                newprof,
            ...
            if not FirefoxProfile.DEFAULT_PREFERENCES:
                with open(
                    os.path.join(
                        os.path.dirname(__file__), WEBDRIVER_PREFERENCES
                    ),
                    encoding='utf-8',
                ) as default_prefs:
                    FirefoxProfile.DEFAULT_PREFERENCES = json.load(
                        default_prefs
                    )
            ...
            for key, value in FirefoxProfile.DEFAULT_PREFERENCES[
                'frozen'
            ].items():
