...
class Log:
    ...
    def __init__(self, driver, bidi_session) -> None:
        ...
        self._mutation_listener_js = _mutation_listener_js_bytes.decode(
            'utf8'
        ).strip()
    ...
    async def mutation_events(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        event: Dict[str, Any] = {}
        async with runtime.wait_for(
            self.devtools.runtime.BindingCalled
        ) as evnt:
            yield event
    ...
    @asynccontextmanager
    async def add_js_error_listener(
        self,
        ...
        async with session.wait_for(
            self.devtools.runtime.ExceptionThrown
        ) as exception:
            yield js_exception
    ...
    async def add_listener(
        ...
        async with session.wait_for(
            self.devtools.runtime.ConsoleAPICalled
        ) as messages:
            yield console

        if (
            event_type == Console.ALL
            or event_type.value == messages.value.type_
        ):
