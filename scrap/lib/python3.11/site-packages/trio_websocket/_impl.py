...
def _get_stream_endpoint(stream, *, local):
    ...
    if socket:
        addr, port, *_ = (
            socket.getsockname() if local else socket.getpeername()
        )
...
class WebSocketConnection(trio.abc.AsyncResource):
    ...
    async def _accept(self, request, subprotocol, extra_headers):
        ...
        await self._send(
            AcceptConnection(
                subprotocol=self._subprotocol, extra_headers=extra_headers
            )
        )
    ...
    async def _get_request(self):
        ...
        if not self.is_server:
            raise RuntimeError(
                'This method is only valid for server connections.'
            )
    ...
    async def _reader_task(self):
        ...
        async with self._send_channel:
            while self._reader_running:
                ...
                if len(data) == 0:
                    logger.debug(
                        '%s received zero bytes (connection closed)', self
                    )
...
class WebSocketServer:
    ...
    def port(self):
        ...
        if len(self._listeners) > 1:
            raise RuntimeError(
                'Cannot get port because this server has'
    ...
    async def run(self, *, task_status=trio.TASK_STATUS_IGNORED):
        ...
        async with trio.open_nursery() as nursery:
            ...
            logger.debug(
                'Listening on %s', ','.join([str(l) for l in self.listeners])
            )
