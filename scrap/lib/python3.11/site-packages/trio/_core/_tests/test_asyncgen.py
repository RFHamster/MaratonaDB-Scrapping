...
async def test_asyncgen_throws_during_finalization(
    ...
    assert (
        'during finalization of async generator' in caplog.records[0].message
    )
...
def test_last_minute_gc_edge_case() -> None:
    ...
    def collect_at_opportune_moment(
        token: _core._entry_queue.TrioToken,
    ) -> None:
...
async def step_outside_async_context(
    aiter_: AsyncGenerator[int, None]
) -> None:
...
async def test_fallback_when_no_hook_claims_it(
    ...
    with restore_unraisablehook():
        ...
        gc_collect_harder()
        assert (
            'awaited something during finalization' in capsys.readouterr().err
        )
