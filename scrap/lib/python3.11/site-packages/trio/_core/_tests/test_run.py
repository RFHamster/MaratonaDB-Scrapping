...
async def test_reschedule() -> None:
    ...
    async def child1() -> None:
        ...
        _core.reschedule(
            not_none(t2), outcome.Error(ValueError('error message'))
        )
        print('child1 exit')
...
async def test_current_time_with_mock_clock(
...
async def test_current_task() -> None:
    ...
    async def child() -> None:
        assert (
            not_none(_core.current_task().parent_nursery).parent_task
            is parent_task
        )
...
def test_system_task_crash_KeyboardInterrupt() -> None:
    ...
    assert RaisesGroup(RaisesGroup(KeyboardInterrupt)).matches(
        excinfo.value.__cause__
    )
...
def test_sniffio_integration() -> None:
    ...
    def alternate_sniffio_library() -> Generator[None, None, None]:
        ...
        prev_library, sniffio.thread_local.name = (
...
def test_setting_strict_exception_groups(
    ...
    async def main() -> None:
        ...
        async with _core.open_nursery(
            **_create_kwargs(open_nursery_strict)
        ) as nursery:
...
async def test_nursery_collapse(strict: bool | None) -> None:
    ...
    maybe_wrapped_runtime_error: type[RuntimeError] | RaisesGroup[
        RuntimeError
...
def test_trio_run_strict_before_started(
    ...
    if start_raiser_strict or (run_strict and start_raiser_strict is None):
        ...
        if start_raiser_strict:
            ...
            assert (
                exc_info.value.message == 'start_raiser nursery custom message'
            )
