...
if TYPE_CHECKING:
    ...
    StatusT_contra = TypeVar(
        'StatusT_contra', contravariant=True, default=None
    )
else:
...
class Deadlines:
    ...
    def expire(self, now: float) -> bool:
        ...
        if (
            len(self._heap)
            > self._active * 2 + DEADLINE_HEAP_MIN_PRUNE_THRESHOLD
        ):
            self._prune()
...
class CancelStatus:
    ...
    _parent: CancelStatus | None = attrs.field(
        default=None, repr=False, alias='parent'
    )

    # All of the CancelStatuses that have this CancelStatus as their parent.
    _children: set[CancelStatus] = attrs.field(
    ...
    abandoned_by_misnesting: bool = attrs.field(
...
def open_nursery(
    ...
    if strict_exception_groups is None:
        strict_exception_groups = (
            GLOBAL_RUN_CONTEXT.runner.strict_exception_groups
        )
...
class Nursery(metaclass=NoPublicConstructor):
    ...
    async def _nested_child_finished(
        ...
        if self._pending_excs:
            try:
                if (
                    not self._strict_exception_groups
                    and len(self._pending_excs) == 1
                ):
        ...
        try:
            ...
            try:
                ...
                async with open_nursery(
                    ...
                    task_status: _TaskStatus[Any] = _TaskStatus(
                        old_nursery, self
                    )
                    thunk = functools.partial(
                        async_fn, task_status=task_status
                    )
...
class Task(metaclass=NoPublicConstructor):
    ...
    def _activate_cancel_status(
        self, cancel_status: CancelStatus | None
    ) -> None:
        if self._cancel_status is not None:
...
class GuestState:
    ...
    def guest_tick(self) -> None:
        ...
        if (
            timeout <= 0
            or isinstance(events_outcome, Error)
            or events_outcome.value
        ):
...
class Runner:
    ...
    def current_statistics(self) -> RunStatistics:
        ...
        * ``io_statistics`` (object): Some statistics from Trio's I/O
        ...
        seconds_to_next_deadline = (
            self.deadlines.next_deadline() - self.current_time()
        )
    ...
    def spawn_impl(
        ...
        *,
        ...
        if context is None:
            context = (
                self.system_context.copy() if system_task else copy_context()
            )
...
def start_guest_run(
    ...
    *done_callback* at some point after :func:`start_guest_run` returns
    ...
    for _tick in range(
        5
    ):  # expected need is 2 iterations + leave some wiggle room
...
elif sys.platform == 'linux' or (
    not TYPE_CHECKING and hasattr(select, 'epoll')
):
