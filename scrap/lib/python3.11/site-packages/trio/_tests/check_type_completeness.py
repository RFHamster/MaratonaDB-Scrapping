...
def check_type(
    ...
    for symbol in current_result['typeCompleteness']['symbols']:
        ...
        for diagnostic in diagnostics:
            ...
            if (
                message not in expected_errors
                and message not in printed_diagnostics
            ):
...
def main(args: argparse.Namespace) -> int:
    ...
    errors_by_platform_file = (
        Path(__file__).parent / '_check_type_completeness.json'
    )
    ...
    for platform in 'Linux', 'Windows', 'Darwin':
        platform_errors = (
            errors_by_platform[platform] + errors_by_platform['all']
        )
        print('*' * 20, f'\nChecking {platform}...')
    ...
    for e in errors_by_platform['Linux'].copy():
        if (
            e in errors_by_platform['Darwin']
            and e in errors_by_platform['Windows']
        ):
            for platform in 'Linux', 'Windows', 'Darwin':
