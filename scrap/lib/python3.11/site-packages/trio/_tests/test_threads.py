...
async def test_do_in_trio_thread() -> None:
    ...
    await check_case(
        from_thread_run_sync, f2, ('error', ValueError), trio_token=token
    )
    ...
    await check_case(
        from_thread_run, f4, ('error', KeyError), trio_token=token
    )
...
async def test_named_thread_os() -> None:
    ...
    async def test_thread_name(name: str, expected: str | None = None) -> None:
        ...
        assert (
            os_thread_name is not None
        ), 'should skip earlier if this is the case'
...
async def test_run_in_worker_thread_cancellation() -> None:
    ...
    async def child(
        q: stdlib_queue.Queue[None], abandon_on_cancel: bool
    ) -> None:
        record.append('start')
        try:
            return await to_thread_run_sync(
                f, q, abandon_on_cancel=abandon_on_cancel
            )
        finally:
            record.append('exit')
...
async def test_run_in_worker_thread_fail_to_spawn(
    ...
    monkeypatch.setattr(
        _core._thread_cache.ThreadCache, 'start_thread_soon', bad_start
    )
...
async def test_trio_from_thread_run_sync_contextvars() -> None:
    ...
    def thread_fn() -> tuple[str, str, str, str, str]:
        ...
        back_parent_value, back_current_value = from_thread_run_sync(
            back_in_main
        )
...
async def test_trio_from_thread_run_contextvars() -> None:
    ...
    def thread_fn() -> tuple[str, str, str, str, str]:
        ...
        back_parent_value, back_current_value = from_thread_run(
            async_back_in_main
        )
        thread_after_value = trio_test_contextvar.get()
        with pytest.raises(sniffio.AsyncLibraryNotFoundError):
...
def test_from_thread_run_during_shutdown() -> None:
    ...
    async def main(use_system_task: bool) -> None:
        save.append(
            agen(_core.current_trio_token() if use_system_task else None)
        )
        await save[-1].asend(None)

    _core.run(
        main, True
    )  # System nursery will be closed and raise RunFinishedError
...
async def test_from_thread_reuses_task() -> None:
    ...
    assert task is await to_thread_run_sync(
        from_thread_run_sync, _core.current_task
    )
    assert task is await to_thread_run_sync(
        from_thread_run, async_current_task
    )
...
async def test_from_thread_check_cancelled() -> None:
    ...
    async def child(abandon_on_cancel: bool, scope: CancelScope) -> None:
        with scope:
            ...
            try:
                return await to_thread_run_sync(
                    f, abandon_on_cancel=abandon_on_cancel
                )
            except _core.Cancelled:
...
async def test_reentry_doesnt_deadlock() -> None:
    ...
    async def child() -> None:
        while True:
            await to_thread_run_sync(
                from_thread_run, sleep, 0, abandon_on_cancel=False
            )
