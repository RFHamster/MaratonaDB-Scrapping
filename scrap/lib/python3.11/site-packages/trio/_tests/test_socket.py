...
class MonkeypatchedGAI:
    def __init__(
        self, orig_getaddrinfo: Callable[..., GetAddrInfoResponse]
    ) -> None:
...
async def test_SocketType_resolve(
    socket_type: AddressFamily, addrs: Addresses
) -> None:
    ...
    with tsocket.socket(family=socket_type) as sock:
        ...
        for local in [False, True]:
            ...
            assert_eq(
                await res((addrs.arbitrary, 'http')), (addrs.arbitrary, 80)
            )
            if v6:
                ...
                sock.setsockopt(
                    tsocket.IPPROTO_IPV6, tsocket.IPV6_V6ONLY, False
                )
                assert_eq(
                    await res(('1.2.3.4', 'http')), ('::ffff:1.2.3.4', 80)
                )
            ...
            if v6 and not gai_without_v4mapped_is_buggy():
                sock.setsockopt(
                    tsocket.IPPROTO_IPV6, tsocket.IPV6_V6ONLY, True
                )
                with pytest.raises(tsocket.gaierror) as excinfo:
            ...
            try:
                ...
                assert (
                    await netlink_sock._resolve_address_nocp(
                        'asdf', local=local
                    )
                    == 'asdf'
                )
