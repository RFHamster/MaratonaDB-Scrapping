...
MemoryStapledStream: TypeAlias = StapledStream[
    MemorySendStream, MemoryReceiveStream
]
...
async def test_resource_busy_errors(client_ctx: SSLContext) -> None:
    ...
    await do_test(
        do_wait_send_all_might_not_block, do_wait_send_all_might_not_block
    )
...
async def test_getpeercert(client_ctx: SSLContext) -> None:
    ...
    assert ('DNS', 'trio-test-1.example.org') in client.getpeercert()[
        'subjectAltName'
    ]
...
async def test_SSLListener(client_ctx: SSLContext) -> None:
    async def setup(
        **kwargs: Any,
        ...
        tsocket.SocketType, SSLListener[SocketStream], SSLStream[SocketStream]
    ]:
