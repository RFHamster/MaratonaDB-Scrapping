            ...
            async def echo_handler(dtls_channel: DTLSChannel) -> None:
                ...
                try:
                    ...
                    async for packet in dtls_channel:
                        print(
                            f'echoing {packet!r} -> {dtls_channel.peer_address!r}'
                        )
    ...
    with endpoint() as server_endpoint, endpoint() as client_endpoint:
        ...
        async with trio.open_nursery() as server_nursery:
            ...
            await server_nursery.start(
                server_endpoint.serve, server_ctx, handler
            )
...
async def test_serve_exits_cleanly_on_close() -> None:
    async with dtls_echo_server(autocancel=False) as (
        server_endpoint,
        address,
    ):
...
async def test_client_multiplex() -> None:
    async with dtls_echo_server() as (_, address1), dtls_echo_server() as (
        _,
        address2,
    ):
        with endpoint() as client_endpoint:
            ...
            async with trio.open_nursery() as nursery:
                with pytest.raises(trio.ClosedResourceError):
                    await nursery.start(
                        client_endpoint.serve, server_ctx, null_handler
                    )
...
async def test_dtls_over_dgram_only() -> None:
    with trio.socket.socket() as s:
        with pytest.raises(
            ValueError, match='^DTLS requires a SOCK_DGRAM socket$'
        ):
            DTLSEndpoint(s)
...
async def test_double_serve() -> None:
    ...
    with endpoint() as server_endpoint:
        ...
        async with trio.open_nursery() as nursery:
            await nursery.start(
                server_endpoint.serve, server_ctx, null_handler
            )
...
async def test_incoming_buffer_overflow(
    ...
    for buffer_size in [10, 20]:
        async with dtls_echo_server() as (_, address):
            with endpoint(
                incoming_packets_buffer=buffer_size
            ) as client_endpoint:
