...
if sys.platform == 'linux':
    ENOTCONN_MSG = (
        r'^\[Errno 107\] (Transport endpoint is|Socket) not connected$'
    )
...
async def test_recv_methods() -> None:
    ...
    with pytest.raises(
        NotImplementedError, match='code will most likely hang'
    ):
        await s2.recv(10)
    ...
    with pytest.raises(
        NotImplementedError, match='^FakeNet send flags must be 0, not'
    ):
