...
@pytest.mark.parametrize(
    'tool', ['pylint', 'jedi', 'mypy', 'pyright_verifytypes']
)
@pytest.mark.filterwarnings(
    # https://github.com/pypa/setuptools/issues/3274
    "ignore:module 'sre_constants' is deprecated:DeprecationWarning",
)
def test_static_tool_sees_all_symbols(
    tool: str, modname: str, tmp_path: Path
) -> None:
...
def test_static_tool_sees_class_members(
    ...
    for class_name, class_ in module.__dict__.items():
        ...
        if tool == 'jedi':
            ...
            completions = script.complete()
            static_names = (
                no_hidden(c.name for c in completions) - ignore_names
            )
...
def test_classes_are_final() -> None:
    ...
    for module in PUBLIC_MODULES:
        for name, class_ in module.__dict__.items():
            ...
            if (
                Protocol in class_.__bases__
                or Protocol_ext in class_.__bases__
            ):
                continue
            ...
            if (
                class_ is trio.abc.Instrument
                or class_ is trio.socket.SocketType
            ):
                continue
