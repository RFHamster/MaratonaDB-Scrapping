...
async def run_process_in_nursery(
    *args: Any, **kwargs: Any
    ...
    async with _core.open_nursery() as nursery:
        ...
        proc: Process = await nursery.start(
...
async def test_warn_on_failed_cancel_terminate(
...
async def test_run_process_internal_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_subprocess_pidfd_unnotified() -> None:
    ...
    async with _core.open_nursery() as nursery:
        ...
        assert (
            noticed_exit.is_set()
        ), "child task wasn't woken after poll, DEADLOCK"
