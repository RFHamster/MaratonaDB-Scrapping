...
def _assert_raises(
    ...
    try:
        ...
        assert (
            wrapped
        ), 'caught exceptiongroup, but expected an unwrapped exception'
...
async def check_two_way_stream(
    ...
    flipped_clogged_stream_maker: Callable[
        [], Awaitable[tuple[Stream, Stream]]
    ] | None
    ...
    async with _ForceCloseBoth(await stream_maker()) as (s1, s2):
        ...
        async def receiver(
            s: Stream, data: bytes | bytearray, seed: int
        ) -> None:
            r = random.Random(seed)
            got = bytearray()
...
async def check_half_closeable_stream(
    ...
    clogged_stream_maker: StreamMaker[HalfCloseableStream, HalfCloseableStream]
    | None,
) -> None:
