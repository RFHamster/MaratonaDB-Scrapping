...
def _family_for(ip: IPAddress) -> int:
    ...
    raise NotImplementedError(
        'Unhandled IPAddress instance type'
    )  # pragma: no cover
...
class FakeNet:
    def __init__(self) -> None:
        ...
        self._auto_ipv4_iter = ipaddress.IPv4Network(
            '1.0.0.0/8'
        ).hosts()  # untested
        self._auto_ipv6_iter = ipaddress.IPv6Network(
            '1::/16'
        ).hosts()  # untested
...
class FakeSocket(trio.socket.SocketType, metaclass=NoPublicConstructor):
    def __init__(
        ...
        if not type:  # pragma: no cover
            ...
            )  # noqa: A001  # name shadowing builtin

        if family not in (trio.socket.AF_INET, trio.socket.AF_INET6):
            raise NotImplementedError(
                f"FakeNet doesn't (yet) support family={family}"
            )
        if type != trio.socket.SOCK_DGRAM:
            raise NotImplementedError(
                f"FakeNet doesn't (yet) support type={type}"
            )
    ...
    async def connect(self, peer: object) -> NoReturn:
        raise NotImplementedError(
            'FakeNet does not (yet) support connected sockets'
        )
    ...
    async def _sendmsg(
        ...
        if flags:
            raise NotImplementedError(
                f'FakeNet send flags must be 0, not {flags}'
            )
    ...
    def getsockopt(
        ...
        self._check_closed()
        raise OSError(
            f"FakeNet doesn't implement getsockopt({level}, {optname})"
        )
    ...
    def setsockopt(
        ...
        raise OSError(
            f"FakeNet doesn't implement setsockopt({level}, {optname}, ...)"
        )
