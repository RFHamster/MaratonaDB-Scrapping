...
_resolver: _core.RunVar[HostnameResolver | None] = _core.RunVar(
    'hostname_resolver'
)
_socket_factory: _core.RunVar[SocketFactory | None] = _core.RunVar(
    'socket_factory'
)
...
def fromfd(
    ...
    family, type_, proto = _sniff_sockopts_for_fileno(
        family, type, proto, index(fd)
    )
...
def _make_simple_sock_method_wrapper(
    ...
    async def wrapper(
        self: _SocketType, *args: P.args, **kwargs: P.kwargs
    ) -> T:
...
async def _resolve_address_nocp(
    ...
    *,
    ...
    if family == _stdlib_socket.AF_INET:
        ...
        hasattr(_stdlib_socket, 'AF_UNIX') and family == _stdlib_socket.AF_UNIX
    ):
        # unwrap path-likes
...
class SocketType:
    ...
    def listen(
        self, /, backlog: int = min(_stdlib_socket.SOMAXCONN, 128)
    ) -> None:
        raise NotImplementedError
...
class _SocketType(SocketType):
    ...
    async def connect(self, address: AddressFormat) -> None:
        ...
        if err != 0:
            raise OSError(
                err, f'Error connecting to {address!r}: {os.strerror(err)}'
            )
    ...
    if sys.platform != 'win32' or (
        ...
        async def sendmsg(
            ...
            if __address is not None:
                __address = await self._resolve_address_nocp(
                    __address, local=False
                )
            return await self._nonblocking_helper(
                _core.wait_writable,
