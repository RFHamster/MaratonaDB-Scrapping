...
def records_untrusted(packet: bytes) -> Iterator[Record]:
    ...
    while i < len(packet):
        try:
            ct, version, epoch_seqno, payload_len = RECORD_HEADER.unpack_from(
                packet, i
            )
...
def decode_handshake_fragment_untrusted(payload: bytes) -> HandshakeFragment:
    ...
    if len(frag) != frag_len:
        raise BadPacket(
            "handshake fragment length doesn't match record length"
        )
...
def challenge_for(
    ...
    packet = encode_record(
        Record(
            ContentType.handshake, ProtocolVersion.DTLS10, epoch_seqno, payload
        )
    )
    return packet
...
async def handle_client_hello_untrusted(
    ...
    if not valid_cookie(endpoint._listening_key, cookie, address, bits):
        ...
        stream = DTLSChannel._create(
            endpoint, address, endpoint._listening_context
        )
        ...
        while True:
            ...
            try:
                ...
                if is_client_hello_untrusted(packet):
                    await handle_client_hello_untrusted(
                        endpoint, address, packet
                    )
                elif address in endpoint._streams:
                    stream = endpoint._streams[address]
                    if stream._did_handshake and part_of_handshake_untrusted(
                        packet
                    ):
...
class DTLSChannel(trio.abc.Channel[bytes], metaclass=NoPublicConstructor):
    ...
    async def _send_volley(
        self, volley_messages: list[_AnyHandshakeMessage]
    ) -> None:
    ...
    async def do_handshake(
        self, *, initial_retransmit_timeout: float = 1.0
    ) -> None:
                ...
                with trio.move_on_after(timeout) as cscope:
                    async for packet in self._q.r:
                        ...
                        if maybe_volley:
                            if (
                                isinstance(
                                    maybe_volley[0], PseudoHandshakeMessage
                                )
                                and maybe_volley[0].content_type
                                == ContentType.alert
                            ):
    ...
    async def send(self, data: bytes) -> None:
        ...
        if not data:
            raise ValueError(
                "openssl doesn't support sending empty DTLS packets"
            )
