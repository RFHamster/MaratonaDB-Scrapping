...
class FrameDecoder:
    ...
    def process_buffer(self) -> Optional[Frame]:
        ...
        frame = Frame(
            self.effective_opcode, payload, finished, self.header.fin
        )
...
class FrameProtocol:
    ...
    def _process_close(self, frame: Frame) -> Frame:
        ...
        if not data:
            ...
            if (
                not isinstance(code, CloseReason)
        ...
        return Frame(
            frame.opcode, data, frame.frame_finished, frame.message_finished
        )
    ...
    def _make_fin_rsv_opcode(
        self, fin: bool, rsv: RsvBits, opcode: Opcode
    ) -> int:
        fin_bits = int(fin) << 7
        rsv_bits = (
